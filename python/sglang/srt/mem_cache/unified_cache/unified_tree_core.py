    ...
    def _insert_walk_step(self, state: _InsertWalkState) -> None:
        ...
        if node.evicted:
            ...
            if dup_start < consumed_from:
                # The request's freshly allocated FULL slots cover the whole
                # duplicate segment, while its SWA slots cover only the part at
                # or after swa_evicted_seqlen.  Keep that CPU-known boundary in
                # the action stream instead of asking the allocator to discover
                # mapped SWA pages with torch.unique on the scheduler thread.
                ...
                if ComponentType.SWA in self.components_by_type:
                    swa_start = max(
                        dup_start,
                        state.params.swa_evicted_seqlen
