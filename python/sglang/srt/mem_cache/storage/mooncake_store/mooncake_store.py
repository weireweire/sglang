...
class MooncakeStore(HiCacheStorage, MooncakeBaseStore):
    ...
    def __init__(
        ...
        self.hybrid_put_stats: dict[str, int] = {}
    ...
    def batch_exists_v2(
        ...
        for transfer in pool_transfers or []:
            ...
            prepared.append(
                (transfer, key_multiplier, start, len(all_component_keys))
            )

        all_exists = (
            self._batch_exist(all_component_keys) if all_component_keys else []
        )
        diagnose_pools = logger.isEnabledFor(logging.DEBUG)
        for transfer, key_multiplier, start, end in prepared:
            # In diagnostics, inspect every pool using the already fetched
            # existence results, even after an earlier pool rules out a hit.
            if not restorable and not diagnose_pools:
                break
            ...
            if boundary and restorable:
                hit_count[transfer.name] = boundary
            pool_restorable_set = set(pool_restorable)
            restorable = [p for p in restorable if p in pool_restorable_set]
            if diagnose_pools:
                logger.debug(
                    "Mooncake lookup pool: first_key=%s pool=%s policy=%s "
                    "queried_pages=%d present_pages=%d pool_boundary=%d "
                    "pool_candidates=%d common_boundary=%d common_candidates=%d",
                    keys[0],
                    transfer.name,
                    transfer.hit_policy,
                    kv_pages,
    ...
    def _batch_io_v2(self, transfers: List[PoolTransfer], is_set: bool):
        ...
        if is_set:
            ...
            for name, _, start, end in prepared:
                missing = [i for i in range(start, end) if exist_result[i] != 1]
                sizes = [all_sizes[i] for i in missing]
                submitted_bytes = sum(
                    sum(size) if isinstance(size, Sequence) else size
                    for size in sizes
                )
                for metric, value in (
                    (f"put_submitted_objects_{name}", len(missing)),
                    (f"put_submitted_bytes_{name}", submitted_bytes),
                ):
                    self.hybrid_put_stats[metric] = (
                        self.hybrid_put_stats.get(metric, 0) + value
                    )
            if missing_idx:
